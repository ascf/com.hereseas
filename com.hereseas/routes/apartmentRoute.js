...
exports.getApartmentList = function(req, res, next) {
    ...
    Apartment.find(
        query,
        'id user_id user_name user_avatar title content cover images type rooms description location create_at update_at').populate('rooms').exec(function(err, apartments) {
        if (err) {
            console.log(err);
        } else {
            res.json({
...
exports.addApartment = function(req, res, next) {
        ...
        apartment.save(function(err, apartment) {
                ...
                if (err) {
                    ...
                    room.bathroom = data.room.bathroom;
